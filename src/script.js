class TodoApp {
    constructor() {
        ...
        this.dropZonePlaceholder = null;
        this.isDragging = false;
        this.currentDropZonePosition = null;
        this.dragGhost = null;
        this.dropZone = null;
        this.lastMouseX = 0;
        this.lastMouseY = 0;
    ...
    getFolderChildren(folderId, limit = 4) {
        ...
        const startDrag = (e) => {
            ...
            const moveDetect = (moveEvent) => {
                const currentX = moveEvent.clientX || (moveEvent.touches && moveEvent.touches[0].clientX);
                const currentY = moveEvent.clientY || (moveEvent.touches && moveEvent.touches[0].clientY);
                
                const distance = Math.sqrt(Math.pow(currentX - startX, 2) + Math.pow(currentY - startY, 2));
                
                if (distance > 5 && !isDragAction) {
                    isDragAction = true;
                    document.removeEventListener('mousemove', moveDetect);
                    ...
                    this.startDrag(containerId, moveEvent);
                }
            };
            ...
            document.addEventListener('touchmove', moveDetect);
            document.addEventListener('mouseup', endDetect);
            document.addEventListener('touchend', endDetect);
        };
    ...
    startDrag(containerId, event) {
        ...
        this.dragGhost = containerElement.cloneNode(true);
        ...
        this.dropZonePlaceholder = document.createElement('div');
        this.dropZonePlaceholder.className = 'drop-zone';
        
        const moveHandler = (e) => {
            e.preventDefault();
            this.handleDragMove(e);
        };
        const endHandler = (e) => this.endDrag(e, moveHandler, endHandler);
    ...
    handleDragMove(event) {
        if (!this.draggedContainer || !this.dragGhost) return;
        ...
        this.dragGhost.classList.remove('over-folder');
        this.dragOverContainer = null;
        
        if (!elementUnder) {
            if (this.dropZonePlaceholder.parentNode) {
                this.dropZonePlaceholder.classList.remove('show');
                setTimeout(() => {
                    if (this.dropZonePlaceholder.parentNode) {
                        this.dropZonePlaceholder.remove();
                    }
                }, 250);
                this.currentDropZonePosition = null;
            }
        ...
        const breadcrumbItem = elementUnder.closest('.breadcrumb-item');
        if (breadcrumbItem) {
            breadcrumbItem.classList.add('drag-over');
            this.dragOverContainer = breadcrumbItem;
            this.lastValidDropTarget = breadcrumbItem;
            if (this.dropZonePlaceholder.parentNode) {
                this.dropZonePlaceholder.classList.remove('show');
                setTimeout(() => {
                    if (this.dropZonePlaceholder.parentNode) {
        ...
        const containerUnder = elementUnder.closest('.container:not(.dragging):not(.drag-ghost)');
        
        if (containerUnder && containerUnder.dataset.id !== String(this.draggedContainer)) {
            const targetId = parseInt(containerUnder.dataset.id);
            const targetContainer = this.containers.find(c => c.id === targetId);
            
            if (targetContainer && targetContainer.type === 'folder') {
                containerUnder.classList.add('drag-over-folder');
                this.dragOverContainer = { type: 'folder', id: targetId };
                this.lastValidDropTarget = { type: 'folder', id: targetId };
                this.dragGhost.classList.add('over-folder');
                if (this.dropZonePlaceholder.parentNode) {
                    this.dropZonePlaceholder.classList.remove('show');
                    setTimeout(() => {
                        if (this.dropZonePlaceholder.parentNode) {
        ...
        const containers = Array.from(wrapper.querySelectorAll('.container:not(.dragging):not(.drag-ghost)'));
        
        let newPosition = null;
        
        if (containers.length === 0 || (clientX < containers[0].getBoundingClientRect().left - 100 && 
            clientY > wrapperRect.top && clientY < wrapperRect.bottom)) {
            newPosition = 'start';
            if (this.currentDropZonePosition !== newPosition) {
                if (!wrapper.firstChild || wrapper.firstChild !== this.dropZonePlaceholder) {
                    wrapper.insertBefore(this.dropZonePlaceholder, wrapper.firstChild);
                }
                setTimeout(() => this.dropZonePlaceholder.classList.add('show'), 10);
                this.dragOverContainer = { type: 'insert', beforeId: containers.length > 0 ? parseInt(containers[0].dataset.id) : null };
                this.lastValidDropTarget = this.dragOverContainer;
                this.currentDropZonePosition = newPosition;
            }
            return;
        }
        ...
        for (let i = 0; i < containers.length; i++) {
            ...
            if (i === 0) {
                if (clientX < currentRect.left && clientX > wrapperRect.left && 
                    clientY > currentRect.top && clientY < currentRect.bottom) {
                    newPosition = `before-${current.dataset.id}`;
                    if (this.currentDropZonePosition !== newPosition) {
                        wrapper.insertBefore(this.dropZonePlaceholder, current);
                        setTimeout(() => this.dropZonePlaceholder.classList.add('show'), 10);
                        this.dragOverContainer = { type: 'insert', beforeId: parseInt(current.dataset.id) };
                        this.lastValidDropTarget = this.dragOverContainer;
                        this.currentDropZonePosition = newPosition;
                    }
                    foundGap = true;
            ...
            if (i < containers.length - 1) {
                ...
                if (Math.abs(currentRect.top - nextRect.top) < 10) {
                    if (clientX > currentRect.right && clientX < nextRect.left &&
                        clientY > currentRect.top && clientY < currentRect.bottom) {
                        newPosition = `between-${current.dataset.id}-${next.dataset.id}`;
                        if (this.currentDropZonePosition !== newPosition) {
                            wrapper.insertBefore(this.dropZonePlaceholder, next);
                            setTimeout(() => this.dropZonePlaceholder.classList.add('show'), 10);
                            this.dragOverContainer = { type: 'insert', afterId: parseInt(current.dataset.id), beforeId: parseInt(next.dataset.id) };
                            this.lastValidDropTarget = this.dragOverContainer;
                            this.currentDropZonePosition = newPosition;
                        }
                        foundGap = true;
            ...
            if (i === containers.length - 1) {
                if (clientX > currentRect.right && clientY > currentRect.top && clientY < currentRect.bottom) {
                    newPosition = `after-${current.dataset.id}`;
                    if (this.currentDropZonePosition !== newPosition) {
                        wrapper.appendChild(this.dropZonePlaceholder);
                        setTimeout(() => this.dropZonePlaceholder.classList.add('show'), 10);
                        this.dragOverContainer = { type: 'insert', afterId: parseInt(current.dataset.id) };
                        this.lastValidDropTarget = this.dragOverContainer;
                        this.currentDropZonePosition = newPosition;
                    }
                    foundGap = true;
        ...
        if (!foundGap && this.dropZonePlaceholder.parentNode) {
            this.dropZonePlaceholder.classList.remove('show');
            setTimeout(() => {
                if (this.dropZonePlaceholder.parentNode) {
                    this.dropZonePlaceholder.remove();
    ...
    endDrag(event, moveHandler, endHandler) {
        ...
        const draggedElement = document.querySelector(`[data-id="${this.draggedContainer}"]`);
        if (draggedElement) {
        ...
        if (this.dropZonePlaceholder && this.dropZonePlaceholder.parentNode) {
            this.dropZonePlaceholder.remove();
        }
        ...
        if (this.dragOverContainer || this.lastValidDropTarget) {
            this.handleDrop();
        }
    ...
    handleDrop() {
        const draggedContainer = this.containers.find(c => c.id === this.draggedContainer);
        if (!draggedContainer) return;
        
        const dropTarget = this.dragOverContainer || this.lastValidDropTarget;
        if (!dropTarget) return;
        
        const oldParentId = draggedContainer.parentId;
        
        if (dropTarget.nodeType === 1 && dropTarget.classList && dropTarget.classList.contains('breadcrumb-item')) {
            const breadcrumbText = dropTarget.textContent.trim();
            
            if (breadcrumbText === 'Home') {
                draggedContainer.parentId = null;
            } else {
                const folderId = dropTarget.dataset.folderId;
                if (folderId && folderId !== 'root') {
                    draggedContainer.parentId = parseInt(folderId);
                } else {
                    draggedContainer.parentId = null;
                }
            }
            
            draggedContainer.lastModified = new Date().toISOString();
            if (draggedContainer.parentId) {
                this.updateFolderTimestamps(draggedContainer.parentId);
            }
            if (oldParentId) {
                this.updateFolderTimestamps(oldParentId);
            }
            this.saveToStorage();
            this.render();
            return;
        ...
        if (dropTarget.type === 'folder') {
            draggedContainer.parentId = dropTarget.id;
            draggedContainer.lastModified = new Date().toISOString();
            this.updateFolderTimestamps(dropTarget.id);
            if (oldParentId) {
                this.updateFolderTimestamps(oldParentId);
            }
            this.saveToStorage();
        ...
        if (dropTarget.type === 'insert') {
            ...
            const draggedIndex = this.containers.findIndex(c => c.id === this.draggedContainer);
            const draggedItem = this.containers.splice(draggedIndex, 1)[0];
            
            let insertIndex;
            
            if (dropTarget.beforeId === null) {
                const firstVisible = visibleContainers[0];
                if (firstVisible) {
                    insertIndex = this.containers.findIndex(c => c.id === firstVisible.id);
                } else {
                    insertIndex = 0;
                }
            } else if (dropTarget.beforeId && dropTarget.afterId) {
                const afterIndex = this.containers.findIndex(c => c.id === dropTarget.afterId);
                insertIndex = afterIndex + 1;
            } else if (dropTarget.beforeId) {
                insertIndex = this.containers.findIndex(c => c.id === dropTarget.beforeId);
            } else if (dropTarget.afterId) {
                const afterIndex = this.containers.findIndex(c => c.id === dropTarget.afterId);
                insertIndex = afterIndex + 1;
            }
            
            if (insertIndex === undefined || insertIndex < 0) {
                insertIndex = this.containers.length;
            }
            
            this.containers.splice(insertIndex, 0, draggedItem);
            draggedItem.lastModified = new Date().toISOString();
            
            if (draggedItem.parentId) {
                this.updateFolderTimestamps(draggedItem.parentId);
            }
        }
